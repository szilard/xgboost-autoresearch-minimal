...
n_workers = 4
per_worker = -(-len(test_100k) // n_workers)
bounds = [(i, min(i + per_worker, len(test_100k))) for i in range(0, len(test_100k), per_worker)]

def prepare_slice(lo_hi):
    lo, hi = lo_hi
    out = [prepare(test_100k.iloc[[i]]) for i in range(lo, hi)]
    return pd.concat([X for X, _ in out]), np.concatenate([y for _, y in out])

label = f"first 100k rows, per-row prepare in {n_workers} processes + batch predict"
...
assert np.array_equal(y_prob, model.predict_proba(prepare(test_100k)[0])[:, 1])
